...
export const TweetMedia = ({ tweet }: { tweet: EnrichedTweet }) => {
  ...
  return (
    <div className="flex flex-1 items-center justify-center">
      ...
      {tweet.photos && (
        <div className="relative flex transform-gpu snap-x snap-mandatory gap-4 overflow-x-auto w-full scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent">
          {tweet.photos.length === 1 ? (
            <img
              key={tweet.photos[0].url}
              src={tweet.photos[0].url}
              width={tweet.photos[0].width}
              height={tweet.photos[0].height}
              title={"Photo by " + tweet.user.name}
              alt={tweet.text}
              className="w-full h-auto max-h-96 rounded-xl border object-contain shadow-sm"
            />
            ...
            <div className="flex gap-2 w-full">
              {tweet.photos.map((photo) => (
                <img
                  ...
                  title={"Photo by " + tweet.user.name}
                  alt={tweet.text}
                  className="flex-1 h-auto max-h-96 rounded-xl border object-cover shadow-sm"
                />
              ))}
            </div>
          ) : (
            <>
              <div className="shrink-0 snap-center w-1" />
              {tweet.photos.map((photo, index) => (
                <img
                  ...
                  width={photo.width}
                  height={photo.height}
                  title={`Photo ${index + 1} by ${tweet.user.name}`}
                  alt={tweet.text}
                  className="h-64 min-w-[90%] shrink-0 snap-center snap-always rounded-xl border object-cover shadow-sm cursor-pointer hover:opacity-90 transition-opacity"
                />
              ))}
              <div className="shrink-0 snap-center w-1" />
            </>
...
export const TweetPreviewCard = ({ tweet }: { tweet: EnrichedTweet }) => {
  // @ts-expect-error package doesn't have type definitions
  ...
  return (
    <a
      href={cardUrl || tweet.url}
      target="_blank"
      rel="noopener noreferrer"
      className="mt-3 block rounded-xl border border-gray-700 bg-gray-800/50 overflow-hidden transition hover:border-gray-600 hover:bg-gray-800"
    >
      {imageUrl && (
        <img
          src={imageUrl}
      ...
      <div className="p-4">
        ...
        {cardUrl && (
          <div className="text-xs text-gray-500 mt-2 truncate">{new URL(cardUrl).hostname}</div>
        )}
      </div>
    </a>
  )
...
export const MagicTweet = ({
  ...
  return (
    ...
    >
      ...
      <TweetPreviewCard tweet={enrichedTweet} />
    </div>
  )
}
